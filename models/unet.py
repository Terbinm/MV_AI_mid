...
def unet_classifier(input_size=(224, 224, 3), base_filters=32,
    ...
    Returns:
        model: 配置為二元分類的U-Net模型，輸出為單一值(1x1x1)
    """
    ...
    # === 添加分類頭 ===
    # 先獲取特徵圖
    feature_map = Conv2D(1, (1, 1), activation='sigmoid')(x)

    # 全局平均池化，將特徵圖壓縮為單一值
    pooled_features = GlobalAveragePooling2D()(feature_map)

    # 最終的二元分類輸出 (1x1x1)
    output = Dense(1, activation='sigmoid')(pooled_features)
...
def get_unet_with_custom_params(config, is_classifier=False):
    """
    根據配置參數構建U-Net模型

    Args:
        config: 包含模型參數的配置對象
        is_classifier: 是否構建分類器模型而非分割模型
