...
def analyze_csv_data(csv_path, output_dir=None):
    ...
    for _, row in df.iterrows():
        ...
        row['error_type'] = '_'.join(error_types) if error_types else 'normal'
...
def create_balanced_sample(csv_path, output_csv_path, max_samples=None, balance_ratio=0.5, random_state=42):
    """
    創建平衡的資料集樣本，確保正常和錯誤樣本比例均衡
    ...
    Returns:
        平衡後的DataFrame
    """
    print(f"創建平衡樣本: {csv_path} -> {output_csv_path}")
    ...
    print(f"原始分佈: 正常={len(normal_df)}, 錯誤={len(error_df)}")
    ...
    # 計算平衡後的錯誤和正常樣本數
    error_samples = int(max_samples * balance_ratio)
    normal_samples = max_samples - error_samples

    # 如果目標樣本數超過原始數量，就用原始數量
    error_samples = min(error_samples, len(error_df))
    normal_samples = min(normal_samples, len(normal_df))
    ...
    balanced_df = balanced_df.sample(frac=1).reset_index(drop=True)  # 打亂順序

    print(f"平衡後分佈: 正常={normal_samples}, 錯誤={error_samples}")
    print(f"平衡後總樣本數: {len(balanced_df)}")

    # 保存平衡後的CSV
    balanced_df.to_csv(output_csv_path, index=False)
    print(f"平衡樣本已保存至: {output_csv_path}")
...
def preprocess_all_data(config):
    ...
    if config.BALANCE_DATASET:
        ...
        balanced_df = create_balanced_sample(
            csv_path,
            balanced_csv_path,
        ...
        # 直接讀取原始CSV
        df = pd.read_csv(csv_path)
        if config.MAX_SAMPLES and config.MAX_SAMPLES < len(df):
            # 隨機抽取指定數量的樣本
    ...
    processor = DatasetProcessor(
        csv_path=csv_path if not config.BALANCE_DATASET else balanced_csv_path,
...
def cleanup_processed_data(config, confirm=True):
    ...
    if confirm:
        confirmation = input(f"確定要刪除所有已處理的資料 ({processed_dir})? [y/N]: ")
